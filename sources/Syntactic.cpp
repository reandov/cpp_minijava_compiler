...
void Syntactic::parse(Lex *scanner, Semantic *semanticAnalyser) throw(AnalysisError) {
...
bool Syntactic::step() throw(AnalysisError) {
    if (currentToken != 0 || currentToken != NULL) {
        std::cout << "<" << currentToken->getLexeme() << ", " << currentToken->getId() << ", " << currentToken->getPosition() << ">" << std::endl;
    }
